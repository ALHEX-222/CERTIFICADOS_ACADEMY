...
export const EditEstudianteModal: React.FC<EditEstudianteModalProps> = ({
  isOpen,
  onClose,
  estudiante,
  onSave,
  ...
  useEffect(() => {
    if (isOpen && estudiante) {
      ...
      setFechaNacimiento(
        estudiante.fecha_nacimiento ? estudiante.fecha_nacimiento.substring(0, 10) : ''
      );
      setDireccion(estudiante.direccion || '');
      setEstado(estudiante.estado || 'activo');
      setImagenExistente(estudiante.imagen_perfil || null);
      setImagenOriginal(estudiante.imagen_perfil || null);
  ...
  const eliminarImagenAnterior = async (rutaImagenAnterior: string) => {
    try {
      await apiClient.delete('/admin/estudiantes/imagen', {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      let imagen_perfil: string | undefined = imagenExistente || undefined;
      ...
      if (selectedFile) {
        ...
        if (imagenOriginal && imagenOriginal !== imagen_perfil) {
